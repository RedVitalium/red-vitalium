...
export default function ProfessionalClinicalHistory() {
  ...
  return <ProfessionalClinicalHistoryInner selectedPatient={selectedPatient} professionalData={professionalData} professionalId={professionalId} isAssigned={isAssigned} notes={notes} medications={medications} bfiResult={bfiResult} mySpecialty={professionalData.specialty} queryClient={queryClient} />;
}
...
function MeasurementsTab({ patientUserId, isAssigned }: { patientUserId: string; isAssigned: boolean }) {
  ...
  const saveMeasuresMutation = useMutation({
    mutationFn: async () => {
      ...
      // Update waist circumference in profiles
...
function ProfessionalClinicalHistoryInner({ selectedPatient, professionalData, professionalId, isAssigned, notes, medications, bfiResult, mySpecialty, queryClient }: any) {
  const navigate = useNavigate();
  const { user } = useAuth();
  const [newNote, setNewNote] = useState("");
  const [isVisibleToOthers, setIsVisibleToOthers] = useState(true);
